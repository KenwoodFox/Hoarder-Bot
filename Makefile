...
# Run the tests
test: build
	docker-compose run hoarder_bot test
...
build:
	# Setup volumes
	mkdir -p ~/HoarderBot-config
	sudo chown -R 1000:1000 ~/HoarderBot-config
	sudo chmod 755 -R ~/HoarderBot-config
	# Build
...
# Get container shell
shell:
	docker-compose run --entrypoint "/bin/bash" hoarder_bot

# Run command in container
run: build
	docker-compose run hoarder_bot $(COMMAND)
